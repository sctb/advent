...
(defun passable (from to steps)
  (let ((from (gethash from steps))
	(to (gethash to steps)))
    (or (null from) (null to) (< from (1- to)))))

(defun egress (pos grid steps)
  "The position of the unvisited end of the pipe at POS"
  (pcase-let ((`(,a . ,b) (pipe-ends pos grid)))
    (cond ((passable pos a steps) a)
	  ((passable pos b steps) b))))

(defun walk (pos grid steps)
  (when-let* ((next (egress pos grid steps))
	      (count (1+ (gethash pos steps))))
    (puthash next count steps)
    (walk next grid steps)))
