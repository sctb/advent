...
(defun actualize (springs broken)
  (let ((springs (copy-sequence springs))
	(b (pop broken)))
    (dotimes (i (length springs))
      (when (eq (aref springs i) ??)
...
(defun arrangements (springs sizes)
  (let* ((need (apply #'+ sizes))
	 ...
	 (arrangements 0))
    (message "Considering %s %s" springs sizes)
    (dolist (comb combs)
      (let* ((broken (at slots comb))
	     (actual (actualize springs broken))
	     (valid (consistent-p actual sizes)))
	(message "%c  %s" (if valid ?+ ? ) actual)
	(when valid (incf arrangements))))
    arrangements))
