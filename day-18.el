...
(defun puzzle-18b ()
  (let* ((max-lisp-eval-depth 9000)
	 ...
	 (n 1)
	 (good n)
	 (bad (length positions))
	 (found nil))
    (while (not found)
      (pcase-let ((`(,height . ,width) (dimensions positions)))
	(let* ((memory (make-grid height width ?.))
	       (scores (grid-like memory empty))
	       (start (cons 0 0))
	       (end (cons (1- height) (1- width))))
	  (drop-bytes memory (seq-take positions n))
	  (step memory scores 0 start end)
	  (if (eq (gref scores end) empty)
	      (if (eq n (1+ good))
		  (setq found (elt positions (1- n)))
		(setq bad n))
	    (setq good n))
	  (setq n (/ (+ bad good) 2)))))
    (format "%s,%s" (cdr found) (car found))))
