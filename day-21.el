...
(defun sequences (seqs pad memo)
  (let ((all nil))
    (dolist (seq seqs)
      (dolist (seq (sequence seq pad memo))
	(push seq all)))
    (shortest all)))

(defun chain (code n memo)
  (let ((seqs (sequence code #'numpad memo)))
    (dotimes (_ n)
      (setq seqs (sequences seqs #'dirpad memo)))
    (car seqs)))
