...
(defun trails (grid scores score pos)
  (let ((min (expt 2 32))
	(trails nil))
    (dolist (pos (options pos))
      (when (eq (gref grid pos) ?.)
	(when-let* ((next (gref scores pos)))
	  (message "POS: %s SCORE: %s NEXT: %s" pos score next)
	  (when (> next score)
	    (setq min (min min next))
	    (push (cons next pos) trails)))))
    (seq-filter (lambda (trail)
		  (eq (car trail) min))
		trails)))

(defun trace (grid scores paths score pos)
  (dolist (trail (trails grid scores score pos))
    (pcase-let ((`(,score . ,pos) trail))
      (gset paths pos ?O)
      (trace grid scores paths score pos))))
...
(defun puzzle-16b ()
  (let* ((grid (read-grid "data/example-16.txt"))
	 ...
	 (start (elt deer 2)))
    (step grid scores deer)
    (trace grid scores paths 0 start)
    (insert-grid paths)
    ;; (count-tiles paths)
    ))
